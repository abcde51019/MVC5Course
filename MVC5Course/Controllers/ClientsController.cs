...
namespace MVC5Course.Controllers
{
    public class ClientsController : Controller
    {
        ...
        public ActionResult Index(int CreditRatingFilter = -1, string LastNameFilter ="",
                                  int pageno = 1)
        {
            var r = (from x in db.Client
                     select x.CreditRating)
                     .Distinct()
                     .OrderBy(x => x);
            ViewBag.CreditRatingFilter = new SelectList(r);

            var n = (from x in db.Client
                     select x.LastName)
                     .Distinct()
                     .OrderBy(x => x);
            ViewBag.LastNameFilter = new SelectList(n);

            var client = db.Client.AsQueryable();
            if (CreditRatingFilter != -1)
            {
                client = client.Where(x => x.CreditRating == CreditRatingFilter);
            }
            if (LastNameFilter != "")
            {
                client = client.Where(x => x.LastName == LastNameFilter);
            }
            ViewData.Model = client.OrderByDescending(x => x.ClientId).ToPagedList(pageno, 10);
            return View();
        }
